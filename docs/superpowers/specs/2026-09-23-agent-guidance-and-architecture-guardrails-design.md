...
`HALT_AND_WAIT` means Agent stops, not that task must enter a terminal state. Operator can later resolve the user decision and use an existing explicit workflow. A `--reason` string alone is not proof of user authority. `HALT_AND_WAIT` is an Agent workflow contract, **not CLI access control**: an Agent with shell access can invoke the existing `decision accept` or `transition SPECIFYING --reason ...` commands itself. No CLI response can prove that the user approved such a call. This design does not authenticate callers or prevent direct file edits; enforcement of user identity requires a separate trusted authorization mechanism. Tests of output and Skills must not be described as an authorization guarantee.
...
A live blocker may carry a Finding id only if task id, `reason_code`, `boundary_ref`, frozen `expected_hash`, and current `actual_hash` match. `SCOPE_DRIFT_*` additionally requires matching baseline/current presence of the referenced boundary (`expected_boundary_present`, `actual_boundary_present`); legacy scope Findings without these optional fields cannot be linked to a Gate blocker. Seal-only `CONTRACT_CHANGED` with equal contract hashes cannot prove event identity and carries no Finding id. Guard recording creates a new Finding when this identity changes and never overwrites the older event. Identical facts at separate times remain one audit identity, not proof of a continuous incident. The defect Finding transition workflow has no close path for this alignment audit record. This design does not add one. A clean recompute leaves the file in place, and later Gates keep ignoring it until a fresh check reproduces it.
...
- `harness align freeze` is explicit operator setup, never an autonomous `HALT_AND_WAIT` recovery step. It validates complete Alignment, current-task Decision closure, and canonical boundary references before changing anything. It then writes matching in-document freeze fields and `alignment-freeze.yaml` atomically enough that a failed validation or write leaves existing artifacts unchanged. It emits no `DECISION:` or `DIRECTIVE:` and cannot transition task state. This command exists because Q2/Q3 task-contract setup otherwise has no CLI route from an unfrozen draft to a seal; it does not authorize a changed frozen contract.
...
- `harness gate` validates task, blocker documents, and convergence metadata before any write. Schema-invalid convergence already fails closed; keep that, and make the failure byte-identical. Compute the decision first, then persist one coherent task write of assessment, state, iteration, and convergence metadata. Task file rename is the commit point; derived telemetry runs afterward. A telemetry or post-commit lock-release failure warns `TELEMETRY_UPDATE_FAILED` but preserves Gate exit `0` and emits the committed `DECISION:`. Task write/rename failure exits `2`, emits no `DECISION:`, and leaves current task bytes unchanged (a temporary file may remain). Other I/O errors also exit `2`; callers inspect persisted task before retrying. Validation failure exits `2` and leaves `.harness/current-task.yaml` byte-identical. Exit `1` remains wrong phase or repairable rejection, not a commit failure.
...
5. Live drift at `GATING` escalates `USER_AUTHORITY_REQUIRED`. A stale `blocked_by` entry, or an open alignment Finding that the fresh check does not reproduce, does not escalate. A Finding with the same code and boundary but different hashes or reversed boundary presence does not lend its id to the new blocker. Current `PASS` converges and deletes the fingerprint. An injected Finding without live drift does not escalate.
...
9. Injected telemetry failure after task rename returns success with committed `DECISION:` and warning. Injected task rename failure leaves task bytes unchanged, returns `2`, and emits no decision.
10. `align freeze` seals a complete draft with no current-task proposed Decision; malformed/incomplete/open-Decision inputs and injected write failures preserve both Alignment artifacts and task bytes. A successful seal allows Context construction without state transition.
...
Phase 2 tests cover exactly one `DIRECTIVE:` on each successful Gate outcome: `CONTINUE` pairs with `RESUME_TYPED_RECOVERY`; `CONVERGED` and `ESCALATED` pair with `NONE`. Wrong-phase/invalid commands emit no directive. Skill instructions must treat missing, unknown, or conflicting directives as stop-and-inspect rather than typed recovery; CLI tests alone cannot prove an LLM obeys instructions. Exercise these cases in Agent evaluation when available. Phases are sequential. Phase 2 does not reopen the Phase 1 write or escalation rules.
