...
def validate_contract(document: dict, *, task_type: str | None) -> None:
    ...
    if document["required"]:
        ...
        if unexpected:
            _invalid(
                f"non-required contract has unsupported field {sorted(unexpected)[0]}"
            )
    ...
    if task_type == "bugfix":
        ...
        if gap and not {"improvement", "missing_information"} <= set(bug_fix):
            _invalid(
                "observability_gap=true requires improvement and missing_information"
            )
        if not gap and has_improvement:
            _invalid(
                "observability_gap=false forbids improvement and missing_information"
            )
    ...
    for proposal in proposals:
        try:
            ...
            code = (
                ...
                else "DIAG_FINDING_INVALID"
            )
            raise ValueError(code) from exc
    if failed and not proposals:
        raise ValueError("DIAG_FINDING_REQUIRED")
    return DiagnosabilityReview(
        document["task"],
        document["contract_required"],
        document["checks"],
...
def validate_review_readiness(
    contract: dict, review: dict, findings: list[dict], *, scope_files: tuple[str, ...]
) -> None:
    ...
    for name in failed:
        if not any(
            name in finding.get("compliance", {}).get("required_checks", [])
            and finding.get("location", {}).get("file") in scope_files
            for finding in linked.values()
        ):
...
def gate_blockers(harness_dir: Path, task: dict, *, head: str, workspace: str):
    ...
    try:
        contract = load_contract(
            harness_dir, task_type=task.get("task", {}).get("type")
        )
    ...
    if risk == "Q3" and contract["applicability"]["reasons"] == ["not_assessed"]:
        return [
            GateBlocker(
                "OBSERVABILITY_CONTRACT_INVALID",
                "implementation",
                "Q3 observability contract is not assessed",
                recover_to="IMPLEMENTING",
            )
        ]
    ...
    if not path.exists():
        return [
            GateBlocker(
                ...
                "verification",
                "missing diagnosability review evidence",
    ...
    try:
        ...
        return [
            GateBlocker(
                ...
                "verification",
                "diagnosability review evidence is invalid or stale",
                recover_to="VERIFYING",
            )
        ]
    return []
...
def validate_compliance_closure(
    finding: dict, record: dict, *, current_head: str, current_workspace: str
) -> None:
    ...
    if (
        ...
        or record.get("workspace_fingerprint_after") != current_workspace
    ):
    ...
    *,
    ...
    if task.get("scope") is None:
        files = tuple(
            sorted(
                set(scope.files)
                | set(contract["applicability"]["inspected_paths"])
    ...
    def equivalent(finding, proposal):
        return finding.get("category") == "diagnosability" and all(
            (
                finding.get(key)
                if key != "required_checks"
                else finding.get("compliance", {}).get("required_checks")
            )
            == proposal[key]
            for key in ("target", "reason_code", "location", "required_checks")
        )

    for proposal in review.proposals:
        match = next(
            (finding for finding in existing if equivalent(finding, proposal)), None
        )
        if match:
            raise ValueError(
                f"DIAG_PROPOSAL_DUPLICATE: {proposal['local_id']} matches {match['id']}"
            )
    next_id = (
        max(
            [
                int(finding["id"].split("-")[1])
                for finding in existing
                if str(finding.get("id", "")).startswith("FND-")
            ]
            or [0]
        )
        + 1
    ...
    mapping = {}
    for proposal in review.proposals:
        fid = f"FND-{next_id:03d}"
        next_id += 1
        mapping[proposal["local_id"]] = fid
        generated.append(
            {
                "id": fid,
    ...
    review_data = {
        ...
        "finding_ids": list(mapping.values()),
    }
    ...
    artifacts.append(
        StagedArtifact(
            "evidence/diagnosability-review.json",
    ...
    publish(
        ...
        replace_paths=frozenset({"evidence/diagnosability-review.json"}),
    )
