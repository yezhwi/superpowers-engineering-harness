...
def make_harness(
    ...
    *,
    ...
    task["risk"] = {
        "level": risk,
        "profile": "STRICT" if risk == "Q3" else "STANDARD",
        "dimensions": {
            "scope": "low",
            "contract": "low",
            "data": "none",
    ...
    contract = {
        ...
        "required": required,
        "applicability": {
            "reasons": ["security"] if required else ["not_assessed"],
    ...
    if required:
        contract.update(
            {
                "business_keys": ["task_id"],
                "failure_boundaries": ["review"],
                "critical_events": ["published"],
            }
        )
...
def make_complete_harness(repo: Path, tmp_path: Path, *, state: str = "GATING") -> Path:
    ...
    return populate_complete_harness(
        repo, make_harness(tmp_path, state=state, observability="not_required")
    )
