...
def setup(p):
 subprocess.run(["git","init","-q"],cwd=p,check=True); cli(p,"init"); subprocess.run(["git","add","-A"],cwd=p,check=True); subprocess.run(["git","commit","-qm","init"],cwd=p,check=True); return p/".harness"
...
def test_transition_rejects_invalid_persisted_task_id(tmp_path):
 h=setup(tmp_path); task=yaml.safe_load((h/'current-task.yaml').read_text()); task['task']['id']='bad-slug'; (h/'current-task.yaml').write_text(yaml.safe_dump(task))
...
def test_requirement_and_invariant_verify_attach_fresh_evidence(tmp_path):
 ...
 (h/'invariants.yaml').write_text(yaml.safe_dump({'invariants':[{'id':'INV-001','statement':'safe','category':'correctness','severity':'critical','status':'pending','verification':[]}]}))
 write_evidence(tmp_path,h,'build')
 assert cli(tmp_path,'requirement','verify','REQ-001','--evidence','build.json').returncode==0
...
def test_verify_rejects_stale_workspace_evidence(tmp_path):
 ...
 assert cli(tmp_path,'requirement','verify','REQ-001','--evidence','build.json').returncode==2
