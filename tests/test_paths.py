...
def test_evidence_path_resolves_canonical_reference_forms(tmp_path):
    ...
    for reference in ("unit-test", "unit-test.json", ".harness/evidence/unit-test.json", str(expected.resolve())):
...
def test_evidence_path_rejects_noncanonical_reference(tmp_path, reference):
    ...
    with pytest.raises(ValueError, match="EVIDENCE_REFERENCE_INVALID.*candidates: fast-green-integration-test"):
