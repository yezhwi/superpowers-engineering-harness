...
def test_bugfix_contract_loads_with_task_type(tmp_path):
    ...
    (harness / "observability.yaml").write_text(yaml.safe_dump({
        ...
        "bug_fix": {"observability_gap": False, "basis": "existing context identifies failure boundary"},
    }))

    assert load_contract(harness, task_type="bugfix")["bug_fix"]["observability_gap"] is False
